...
| Name | Format | S | D | Description | Example
| ---- | ------ | ---------------- | --------- | ----------- | -------
| `placeholder` |  `%placeholder% <operation> string`. Operation can be any of: `=`, `<`, `>`, `<=`, `>=`, `==` | no | no | Compares the PlaceholderAPI placeholder with the string via the operation. `==` doesn't convert the data type of the string or placeholder. | `placeholder %server_tps% >= 19`
