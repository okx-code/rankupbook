...
| Name | Format | S | D | Description | Example
| ---- | ------ | ---------------- | --------- | ----------- | -------
| `placeholder` | text | no | no | `%placeholder% <operation> string`<br>Operation can be any of:<br>`=`, `<`, `>`, `<=`, `>=`, `==`Compares the PAPI with the<br>string via the operation.<br>`==` doesn't convert<br>the data type of<br>the string<br>or placeholder. | `placeholder %server_tps% >= 19`
